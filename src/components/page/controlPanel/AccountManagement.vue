<template>
    <div id="accountindex" class="table">
		...
		<div>
			<input type="number" placeholder="用户名" v-model="mobile"/>
			<input type="password" placeholder="密码" v-model="password"/>
			<button @click="login()">登录</button>
		</div>
		<button @click="getList">点击获取学校列表</button>
        <div class="container">
...
// import {getList,getListAPI} from '../../../utils/api.js'
let qs = require('qs');
export default {
    components:{
        MyModal
    ...
    methods:{
		login(){
			...
			this.$post("admin/Login/login",qs.stringify(params)).then(res=>{
				console.log(res,123)
				this.$message.success(res.msg)
				var token = sessionStorage.setItem('mtToken',res.data.mtToken)
			})
		},
		getList(){
			let myToken = sessionStorage.getItem('mtToken')
			// this.$fetch("admin/School/schoolList?mtToken=94ad24647fcb43ac0c6697b98f483b1a",{}).then(res=>{
			// 	console.log(res,123)
			// })
			this.$fetch("admin/School/schoolList?mtToken="+myToken,{}).then(res=>{
				console.log(res,123)
			})
		},
        // 账号列表
	...
	created(){
		...
		// 			console.log(res,123)
		// 		})
		//         // getList(data).then(res => console.log(res))
		//       },
		// let params = {
		// 	page:1,
		// 	limit:10,
		// };
		this.$fetch("admin/login/dataList.html",{}).then(res=>{
			console.log(res,123)
		})
		// let params = {
		...
		// this.$post("/jeefast-ioszhibo/jxNoteHomeworkDetails/list",formData).then(res=>{
		// 	console.log(res,123)
		// })
	}
}
